...
ES_WIFI_Status_t ES_WIFI_CloseServerConnection(ES_WIFIObject_t *Obj, int socket)
{
  ...
  if(ret != ES_WIFI_STATUS_OK)
  {
    DEBUGMSG(" Can not select socket %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
...
ES_WIFI_Status_t ES_WIFI_StopServerSingleConn(ES_WIFIObject_t *Obj, int socket)
{
  ...
  if(ret != ES_WIFI_STATUS_OK)
  {
    DEBUGMSG("Selecting socket failed: %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
  ...
  if(ret != ES_WIFI_STATUS_OK)
  {
    DEBUGMSG("Stopping server failed %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
  ...
  *SentLen = Reqlen;
  ...
  if(ret == ES_WIFI_STATUS_OK)
  {
    ...
    if(ret == ES_WIFI_STATUS_OK)
    {
      ...
      {
        DEBUGMSG("Send Data command failed\n");
      }
    }
    else
    {
      DEBUGMSG("S2 command failed\n");
    }
  }
  else
  {
    ...
    if(ret == ES_WIFI_STATUS_OK)
    {
      ...
      if(ret == ES_WIFI_STATUS_OK)
      {
        ...
        if(ret == ES_WIFI_STATUS_OK)
        {
          ...
          if (ret != ES_WIFI_STATUS_OK)
          {
            DEBUGMSG("AT_RequestReceiveData  failed\n");
          }
        }
        else
        {
         DEBUGMSG("setting timeout failed\n");
        }
      }
      else
      {
